...
def is_admin(user_id: int) -> bool:
    ...
    # ================================================================
# ================================================================
# PART 2 — SAFE REPLY / UI / START / MENU / STATE MACHINE / ROUTER
# ================================================================
...
async def safe_reply(update: Update, text: str, **kwargs):
    """
    Safely reply to messages or callback queries.
    Prevents crashes when update.message is None.
    """
...
def manage_accounts_kb(accounts):
    ...
    for acc in accounts:
        ...
        phone = acc[2]
        kb.append([
            InlineKeyboardButton(f"📱 {phone}", callback_data=f"acc_show:{acc_id}"),
    ...
    kb.append([InlineKeyboardButton("🔙 Back", callback_data="ui_menu")])
    return InlineKeyboardMarkup(kb)
...
def admin_panel_kb():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🔙 Back", callback_data="ui_menu")]
    ])
...
# --------------------------------------------------
# STATE MACHINE (user_states table)
# --------------------------------------------------
def set_state(user_id: int, state: str, temp: dict = None):
    run_db(
        "INSERT OR REPLACE INTO user_states (user_id, state, temp_data) VALUES (?, ?, ?)",
        (user_id, state, json.dumps(temp) if temp else None),
    )


def get_state(user_id: int):
    row = run_db(
        "SELECT state, temp_data FROM user_states WHERE user_id = ?",
        (user_id,), fetch="one"
    )
    if not row:
        return None, None
...
# --------------------------------------------------
# MESSAGE ROUTER — FIXED & FULL ADMIN INPUT LOGIC
# --------------------------------------------------
async def message_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.callback_query:
        return  # callback handled in Part 4
    ...
    if state == "admin_make_code":
        ...
        try:
            ...
            return await safe_reply(update, "❌ Both must be numbers.")
    ...
    if state == "admin_broadcast_msg":
        ...
        for u in users:
            try:
                await context.bot.send_message(int(u[0]), text)
                sent += 1
            except:
                pass
