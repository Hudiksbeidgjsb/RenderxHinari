...
async def callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "add_account":
        ensure_user(uid)
        # free users can add only 1 account
        accounts = get_accounts(uid)
        if not is_premium(uid) and len(accounts) >= 1:
            return await safe_edit(update,
                "Free users can only add 1 account.\n\n"
                "Buy premium for unlimited accounts. Contact @NOTCH2ND"
            )
        set_state(uid, "login_api_id", {})
        return await safe_edit(update, "Send your *API ID*:", parse_mode="Markdown")
    ...
    if data == "manage_accounts":
        ...
        kb = []
        for acc in accounts:
            acc_id, session_file, phone, api_id = acc
            kb.append([InlineKeyboardButton(f"{phone} ({acc_id})", callback_data=f"account_show:{acc_id}")])
    ...
    if data.startswith("account_show:"):
        ...
        kb = [
            [InlineKeyboardButton("🗑 Delete Account", callback_data=f"account_delete:{acc_id}")],
    ...
    if data.startswith("account_delete:"):
        ...
        return await safe_edit(
            ...
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Back", callback_data="manage_accounts")]])
        )
    ...
    # ================================
    if data == "redeem_code":
        set_state(uid, "redeem_wait_code", {})
    ...
    # ================================
    if data == "settings":
        delay = get_user_delay(uid)
        kb = [
            [InlineKeyboardButton("⏳ Set Delay", callback_data="set_delay_menu")],
            [InlineKeyboardButton("⬅️ Back", callback_data="menu")],
        ]
        return await safe_edit(
            update,
            f"⚙️ *Settings*\n\n"
            f"Current delay: *{delay//60} minutes*",
            reply_markup=InlineKeyboardMarkup(kb),
            parse_mode="Markdown"
        )
    ...
    if data == "set_delay_menu":
        ...
        return await safe_edit(
            update,
            "⏱ *Forwarding Delay*\n\n"
            "Choose how often your saved messages will be forwarded.\n\n"
    ...
    if data.startswith("delay_"):
        ...
        run_db("UPDATE users SET delay_seconds=? WHERE user_id=?", (secs, uid))

        kb = [[InlineKeyboardButton("⬅️ Back", callback_data="settings")]]
