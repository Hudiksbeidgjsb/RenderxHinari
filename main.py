...
async def login_flow_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str):
    ...
    if state == "waiting_api_id":
        ...
        return await update.message.reply_text("🔑 Now send your API HASH:")
    ...
    if state == "waiting_phone":
        ...
        try:
            ...
            clear_user_state(user_id)
            return await update.message.reply_text(f"❌ Error sending code: {e}")
    ...
    if state == "waiting_code":
        ...
        try:
            ...
            return await update.message.reply_text("✅ Account connected successfully!")
        except Exception as e:
            return await update.message.reply_text(f"❌ Code error: {e}")
    ...
    if state == "waiting_2fa":
        ...
        try:
            ...
            clear_user_state(user_id)
            create_or_refresh_user(user_id)
            return await update.message.reply_text("🔐 Logged in with 2FA!")
        except Exception as e:
            delete_account(temp.get("acc_id"))
            clear_user_state(user_id)
            return await update.message.reply_text(f"❌ 2FA error: {e}")
...
async def message_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # admin states
    if user_id in ADMIN_IDS and state and state.startswith("admin_"):
