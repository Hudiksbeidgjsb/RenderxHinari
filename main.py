...
async def safe_reply(update: Update, text: str, **kwargs):
    """
    Safely reply to either message or callback query.
...
def manage_accounts_kb(accounts):
    ...
    rows.append([InlineKeyboardButton("➕ Add Account", callback_data="ui_add_account")])
    rows.append([InlineKeyboardButton("🔙 Back", callback_data="ui_menu")])
...
async def message_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state and state.startswith("login_"):
        return await handle_login_states(update, context, state, payload)
