...
async def safe_edit(update: Update, text: str, **kwargs):
    ...
    Accepts the same kwargs as reply_text/edit_message_text (parse_mode, reply_markup).
    """
    q = update.callback_query
    if q:
        try:
            # prefer to edit the original callback message
            return await q.edit_message_text(text, **kwargs)
        except Exception:
            # fallback to answer popup if editing failed
            try:
                await q.answer(text)
            except:
                pass
            # final fallback: send as a normal message
            return await safe_reply(update, text, **kwargs)
    # fallback for non-callback updates
    return await safe_reply(update, text, **kwargs)
...
async def callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "ui_add_account":
        ...
        # free users limit check (uses premium_days_left to decide premium)
        if not is_premium_active(uid) and len(accounts) >= 1:
            return await safe_edit(
                update,
                "Free users can only add 1 account.\n\nBuy premium for more accounts. Contact @NOTCH2ND"
            )
    ...
    if data == "ui_manage_accounts":
        accounts = get_accounts(uid)
        if not accounts:
            # show same Add Account button inside manage UI
        ...
        # build manage keyboard using your helper
        return await safe_edit(
            update,
            "📂 *Your Accounts:*",
    ...
    # Show single account (callback_data created by manage_accounts_kb: acc_show:<id>)
    if data and data.startswith("acc_show:"):
        try:
        ...
        # acc tuple: (account_id, owner_id, phone, api_id, api_hash, session_file, is_forwarding, created_at)
    ...
    # Toggle start/stop forwarding
    if data and data.startswith("acc_toggle:"):
        try:
            acc_id = int(data.split(":", 1)[1])
        except:
            return await safe_edit(update, "Invalid account id.")
        ...
        if not is_fw:
            ...
            if started:
                return await safe_edit(update, "✅ Forwarding started.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Back", callback_data="ui_manage_accounts")]]))
            else:
                return await safe_edit(update, "❌ Failed to start forwarding.")
        else:
            await stop_forward_for_account(acc_id)
            return await safe_edit(update, "⏸ Forwarding stopped.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Back", callback_data="ui_manage_accounts")]]))

    # Delete account
    if data and data.startswith("acc_delete:"):
        try:
            acc_id = int(data.split(":", 1)[1])
        except:
            return await safe_edit(update, "Invalid account id.")
        ...
        return await safe_edit(update, "🗑 Account deleted.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Back", callback_data="ui_manage_accounts")]]))

    # ---------- REDEEM ----------
    if data == "ui_redeem":
        # set the state name that message_router expects
    ...
    if data == "ui_settings":
        ...
        delay = (row[4] if row else 300)  # delay_setting column (seconds)
        kb = InlineKeyboardMarkup([
        ...
        return await safe_edit(
            update,
            f"⚙️ *Settings*\n\nCurrent forwarding delay: *{delay//60} minutes*",
            reply_markup=kb,
            parse_mode=ParseMode.MARKDOWN
        )
    ...
    if data == "ui_set_delay":
        ...
        return await safe_edit(
            update,
            "⏱ *Forwarding Delay*\n\nChoose how often the saved messages will be forwarded.\n\n⚠️ Short delays may increase Telegram limits.",
            reply_markup=kb,
            parse_mode=ParseMode.MARKDOWN
        )

    if data and data.startswith("delay_"):
        try:
        ...
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Back", callback_data="ui_settings")]])
        return await safe_edit(update, f"✅ Delay updated to *{secs//60} minutes*.", reply_markup=kb, parse_mode=ParseMode.MARKDOWN)

    # ---------- ADMIN PANEL ----------
    if data == "ui_admin_panel":
        if not is_admin(uid):
            return await safe_edit(update, "❌ You are not authorized.")
        return await safe_edit(update, "👑 *Admin Panel*", reply_markup=admin_panel_kb(), parse_mode=ParseMode.MARKDOWN)

    # Admin sub actions (names match admin_panel_kb)
    if data == "admin_stats":
        if not is_admin(uid):
        ...
        return await safe_edit(update, f"📊 *Stats*\nUsers: {users}\nAccounts: {accounts}", parse_mode=ParseMode.MARKDOWN)

    if data == "admin_gencode":
        if not is_admin(uid):
            return await safe_edit(update, "❌ Not authorized.")
        set_state(uid, "admin_make_code", {})
        return await safe_edit(update, "🎟 Format: `<quantity> <days>`\nExample: `5 30`", parse_mode=ParseMode.MARKDOWN)

    if data == "admin_ban":
        if not is_admin(uid):
            return await safe_edit(update, "❌ Not authorized.")
        set_state(uid, "admin_ban_user", {})
        return await safe_edit(update, "⛔ Send user ID to ban:")

    if data == "admin_unban":
        if not is_admin(uid):
            return await safe_edit(update, "❌ Not authorized.")
        set_state(uid, "admin_unban_user", {})
        return await safe_edit(update, "♻️ Send user ID to unban:")

    if data == "admin_extend":
        if not is_admin(uid):
            return await safe_edit(update, "❌ Not authorized.")
        set_state(uid, "admin_extend_user", {})
        return await safe_edit(update, "⭐ Send: `<user_id> <days>`")

    if data == "admin_broadcast":
        if not is_admin(uid):
            return await safe_edit(update, "❌ Not authorized.")
        set_state(uid, "admin_broadcast_msg", {})
        return await safe_edit(update, "📢 Send broadcast message:")
